...
def install_lib_ifneeded(
        name: str,
        url: str,
        commit: str,
        is_pkg_config: bool = False,
        is_ld_library: bool = False,
        is_cdpath: bool = False,
    ...
    if operating_system == 'Darwin':
        ...
        shell(f'./configure --prefix={INSTALL_DIR}', cwd=lib_dir, env=environment)
    elif operating_system.find('MINGW') > -1:
        shell(f'./autogen.sh CC=gcc CXX=g++ --prefix={INSTALL_DIR}', cwd=lib_dir, env=environment)
...
def install_name_tool(
        option: str,
        library: str,
        binary_or_library: str,
        library_dir: str = f'{INSTALL_DIR}/lib'):
...
def change_dylib_path_to_relative():
    ...
    libimobiledevice_glue='libimobiledevice-glue-1.0.0.dylib'
    ...
    install_name_tool('-change', libimobiledevice_glue, libusbmuxd)

    print(f'✅ {libimobiledevice}\n')
    for library in [libssl, libcrypto, libplist, libusbmuxd]:
        install_name_tool('-change', library, libimobiledevice, library_dir=f'{ROOT_PATH}/build/lib')
...
def build_libimobiledevice(
    ...
    cython = "" if enable_cython else "--without-cython"
    debug = "--enable-debug" if enable_debug else ""
    ...
    if operating_system == 'Darwin':
        # For some reason the first time it set the libtool folter to ../.. instead of .
        # so running a second time the issue its fixed
        shell(f'./autogen.sh --prefix={build_path} {cython} {debug}', env=environment)
        # shell(f'./autogen.sh --prefix={build_path} {cython} {debug}')
        # shell(f'./configure --prefix={INSTALL_DIR}', cwd=lib_dir, env=environment)
    elif operating_system.find('MINGW') > -1:
        shell(f'./autogen.sh CC=gcc CXX=g++ --prefix={build_path} {cython} {debug}', env=environment)
